...
let create_documents word_list =
  ...
  let desserialize word_list = 
    let index = take_in_between word_list ".I" ".T" |> int_of_string_singleton in 
  ...
  let rec desserialize_while_not_empty words = 
    match (desserialize words) with
      ...
      | document, next_words -> documents := document::(!documents); desserialize_while_not_empty next_words
  in
